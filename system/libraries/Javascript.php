...
class Core_Javascript {
	...
	public function __construct($params = array())
	{
		...
		$this->O2->load->library('javascript/'.$js_library_driver, array('autoload' => $autoload));
		// make js to refer to current library
		$this->js =& $this->O2->$js_library_driver;
	...
	function external($external_file = '', $relative = FALSE)
	{
		...
		{
			if ($this->O2->config->item('javascript_location') != '')
			{
				$this->_javascript_location = $this->O2->config->item('javascript_location');
			}
		}
		...
		{
			$str = $this->_open_script($this->O2->config->slash_item('base_url').$this->_javascript_location.$external_file);
		}
	...
	{
		$str = '<script type="text/javascript" charset="'.strtolower($this->O2->config->item('charset')).'"';
		$str .= ($src == '') ? '>' : ' src="'.$src.'">';
		return $str;
	}
