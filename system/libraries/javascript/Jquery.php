...
class Core_Jquery extends Core_Javascript {
	...
	{	
		...
		$controller = (strpos('://', $controller) === FALSE) ? $controller : $this->O2->config->site_url($controller);
		
		// ajaxStart and ajaxStop are better choices here... but this is a stop gap
		if ($this->O2->config->item('javascript_ajax_img') == '')
		{
			$loading_notifier = "Loading...";
		}
		else
		{
			$loading_notifier = '<img src=\'' . $this->O2->config->slash_item('base_url') . $this->O2->config->item('javascript_ajax_img') . '\' alt=\'Loading\' />';
		}
	...
	function _compile($view_var = 'script_foot', $script_tags = TRUE)
	{
		...
		$this->O2->load->vars(array('library_src' => $external_scripts));
		...
		$this->O2->load->vars(array($view_var => $output));
