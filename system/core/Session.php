...
class Core_Session 
{
	...
	public function __construct()
	{
		...
		$this->_config = $CFG->item('session');
		$this->_config->encryption_key = $CFG->item('encryption_key');

		$this->_config->cookie = $CFG->item('cookie');

		// Set Session Name
		$this->_name = $this->_config->cookie->name.'_'.$URI->active->app->name;
	...
	public function read()
	{
		...
		{
			$this->system_data = json_decode($_COOKIE_SESSION);
		}

		if($_COOKIE_SESSION = $IN->cookie($this->_name) AND empty($this->userdata))
		{
			$this->userdata = json_decode($_COOKIE_SESSION);
		}
		...
		if($this->_apc_support === TRUE AND $this->_config->use_apc === TRUE)
		{
			...
			$_APC_SESSION = json_decode($_APC_SESSION);
	...
	public function update()
	{
		...
		{
			$old_system_data = json_decode($_OLD_SESSION);
		}
	...
	private function write()
	{
		...
		if($this->_config->use_native === TRUE)
		{
			...
			$_SESSION[$this->_name] = $this->userdata;
		...
		if($this->_apc_support === TRUE AND $this->_config->use_apc === TRUE)
		{
			...
			$_APC_SESSION[$this->_name] = $this->userdata;
