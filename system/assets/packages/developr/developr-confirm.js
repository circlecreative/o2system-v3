	...
	$.fn.confirm = function(options)
	{
		return this.each(function()
		{
			...
			if (modeTooltip)
			{
				...
				// Confirm button
				confirmButton = $('<button type="button" class="'+['btn btn-xs'].concat(settings.confirmClasses).join(' ')+'">'+settings.confirmText+'</button>').click(confirmFunc);

				// Cancel button
				if (settings.cancel)
				{
					// Create
					cancelButton = $('<button type="button" class="'+['btn btn-xs'].concat(settings.cancelClasses).join(' ')+'">'+settings.cancelText+'</button>').click(cancelFunc);

					// Insert
					if (settings.cancelFirst)
					{
						cancelButton.addClass('btn-danger mid-margin-right').appendTo(message);
