                         ...
                         $run2_data = [ ] )
{
	...
	if ( ! empty( $rep_symbol ) )
	{
		$run1_data = xhprofTrimRun( $run1_data, [ $rep_symbol ] );
		if ( $diff_mode )
		{
			$run2_data = xhprofTrimRun( $run2_data, [ $rep_symbol ] );
		}
	}
	...
	$base_url_params = xhprofArrayUnset(
		xhprofArrayUnset(
			$url_params,
			'symbol' ),
		'all' );
	...
	if ( ! empty( $rep_symbol ) )
	{
		...
		if ( $diff_mode )
		{
			...
			symbolReport(
				$url_params, $run_delta, $symbol_tab[ $rep_symbol ],
				$sort, $rep_symbol,
				$run1, $info1,
				$run2, $info2 );
		}
		...
		{
			symbolReport(
				$url_params, $run1_data, $symbol_tab[ $rep_symbol ],
...
function printFunctionInfo( $url_params, $info, $sort, $run1, $run2 )
{
	...
	foreach ( $metrics as $metric )
	{
		// Inclusive metric
		printTdNum(
			$info[ $metric ], $format_cbk[ $metric ],
			( $sort_col == $metric ) );
		printTdPct(
			$info[ $metric ], $totals[ $metric ],
			( $sort_col == $metric ) );
...
function pcInfo( $info, $base_ct, $base_info, $parent )
{
	...
	foreach ( $metrics as $metric )
	{
		...
		printTdPct(
			$info[ $metric ], $base_info[ $metric ], ( $sort_col == $metric ),
			getTooltipAttributes( $type, $metric ) );
	}
}
                       ...
                       $symbol_info2 = NULL )
{
	...
	{
		printTdNum(
			$symbol_info[ "excl_" . $metric ], $format_cbk[ "excl_" . $metric ],
			( $sort_col == $metric ),
			getTooltipAttributes( "Child", $metric ) );
		printTdPct(
			$symbol_info[ "excl_" . $metric ], $symbol_info[ $metric ],
			( $sort_col == $metric ),
			getTooltipAttributes( "Child", $metric ) );
	}
	print( "</tr>" );
	...
	{
		printPcArray(
			$url_params, $results, $base_ct, $base_info, TRUE,
			$run1, $run2 );
	}
	...
	{
		printPcArray(
			$url_params, $results, $base_ct, $base_info, FALSE,
			$run1, $run2 );
	}
                                  ...
                                  $run )
{
	...
	profilerReport(
		$url_params, $rep_symbol, $sort, $run, $run_desc,
		$xhprof_data );
}
                              ...
                              $run, $wts, $symbol, $sort, $run1, $run2 )
{
	...
	{                              // specific run to display?
		...
		{
			$xhprof_data = $xhprof_runs_impl->getRun(
				$runs_array[ 0 ],
				$source,
				$description );
		}
