 ...
 * @author Kannan
 */
function xhprofEetPossibleMetrics() {
 ...
 * @author Kannan
 */
function initMetrics($xhprof_data, $rep_symbol, $sort, $diff_report = false) {
	global $stats;
	global $pc_stats;
	global $metrics;
	global $diff_mode;
 ...
 * @author Kannan
 */
function xhprofGetMetrics($xhprof_data) {
 ...
 * @author Kannan
 */
function xhprofParseParentChild($parent_child) {
 ...
 * @author Kannan
 */
function xhprofBuildParentChildKey($parent, $child) {
	if ($parent) {
		return $parent . "==>" . $child;
	} else {
		return $child;
 ...
 *  @author Kannan
 */
function xhprofValidRun($run_id, $raw_data) {

	$main_info = $raw_data["main()"];
	if (empty($main_info)) {
		xhprofError("XHProf: main() missing in raw data for Run ID: $run_id");
		return false;
	}
	...
	if (isset($main_info["wt"])) {
		...
		xhprofError("XHProf: Wall Time information missing from Run ID: $run_id");
		return false;
	}
	...
	foreach ($raw_data as $info) {
		...
		// basic sanity checks...
		if ($val < 0) {
			xhprofError("XHProf: $metric should not be negative: Run ID $run_id"
			             . serialize($info));
			return false;
		}
		if ($val > (86400000000)) {
			xhprofError("XHProf: $metric > 1 day found in Run ID: $run_id "
			             . serialize($info));
			return false;
		}
	}
 ...
 * @author Kannan
 */
function xhprofTrimRun($raw_data, $functions_to_keep) {
 ...
 * @author Kannan
 */
function xhprofNormalizeMetrics($raw_data, $num_runs) {
	...
	if (isset($raw_data["==>main()"]) && isset($raw_data["main()"])) {
		xhprofError("XHProf Error: both ==>main() and main() set in raw data...");
	}
 ...
 *  @author Kannan
 */
function xhprofAggregateRuns($xhprof_runs_impl, $runs,
							 $wts, $source="phprof",
	...
	foreach ($runs as $idx => $run_id) {
		...
		if (!xhprofValidRun($run_id, $raw_data)) {
			$bad_runs[] = $run_id;
			continue;
		}
		...
		if ($use_script_name) {
			...
			if ($page) {
				...
				$raw_data[xhprofBuildParentChildKey("main()",
				                                        "__script::$page")]
					= $fake_edge;
			} else {
				$use_script_name = false;
			...
			if ($use_script_name) {
				...
				if (substr($parent_child, 0, 9) == "main()==>") {
					...
					if (substr($child, 0, 10) != "__script::") {
						$parent_child = xhprofBuildParentChildKey("__script::$page",
						                                              $child);
					}
				}
			}
	...
	$data['raw'] = xhprofNormalizeMetrics($raw_data_total,
	                                        $normalization_count);
 ...
 */
function xhprofComputeFlatInfo($raw_data, &$overall_totals) {

	global $display_calls;

	$metrics = xhprofGetMetrics($raw_data);
	...
	$symbol_tab = xhprofComputeInclusiveTimes($raw_data);
 ...
 * @author Kannan
 */
function xhprofComputeDiff($xhprof_data1, $xhprof_data2) {
	global $display_calls;

	// use the second run to decide what metrics we will do the diff on
	$metrics = xhprofGetMetrics($xhprof_data2);
 ...
 * @author Kannan
 */
function xhprofComputeInclusiveTimes($raw_data) {
	global $display_calls;
	...
	foreach ($raw_data as $parent_child => $info) {
		...
		if ($parent == $child) {
			 ...
			 */
			xhprofError("Error in Raw Data: parent & child are both: $parent");
			return;
		}
 ...
 *  @author Kannan
 */
function xhprofPruneRun($raw_data, $prune_percent) {

	$main_info = $raw_data["main()"];
	if (empty($main_info)) {
		xhprofError("XHProf: main() missing in raw data");
		return false;
	}
	...
	if (isset($main_info["wt"])) {
		...
		xhprofError("XHProf: for main() we must have either wt "
	...
	initMetrics($raw_data, null, null, false);
	$flat_info = xhprofComputeInclusiveTimes($raw_data);
	...
	foreach ($raw_data as $parent_child => $info) {
		...
		if ($flat_info[$child][$prune_metric] < $prune_threshold) {
			...
			// be a dangling child.
			// So instead change its parent to be a special functionPruned().
			$pruned_edge = xhprofBuildParentChildKey("__pruned__()", $child);
 ...
 * @author Kannan
 */
function xhprofArraySet($arr, $k, $v) {
	$arr[$k] = $v;
	return $arr;
}
 ...
 * @author Kannan
 */
function xhprofArrayUnset($arr, $k) {
	unset($arr[$k]);
	return $arr;
}
 ...
 * @author Kannan
 */
function xhprofGetParamHelper($param) {
	$val = null;
	if (isset($_GET[$param]))
		$val = $_GET[$param];
 ...
 * @author Kannan
 */
function xhprofGetStringParam($param, $default = '') {
	$val = xhprofGetParamHelper($param);
 ...
 * @author Kannan
 */
function xhprofGetUintParam($param, $default = 0) {
	$val = xhprofGetParamHelper($param);
	...
	xhprofError("$param is $val. It must be an unsigned integer.");
	return null;
}
 ...
 * @author Kannan
 */
function xhprofGetFloatParam($param, $default = 0) {
	$val = xhprofGetParamHelper($param);
	...
	xhprofError("$param is $val. It must be a float.");
	return null;
}
 ...
 * @author Kannan
 */
function xhprofGetBoolParam($param, $default = false) {
	$val = xhprofGetParamHelper($param);
	...
	switch (strtolower($val)) {
		...
		case 'no':
			...
			break;
		default:
			xhprofError("$param is $val. It must be a valid boolean string.");
			return null;
	}
 ...
 * @author Kannan
 */
function xhprofParamInit($params) {
	/* Create variables specified in $params keys, init defaults */
	foreach ($params as $k => $v) {
		switch ($v[0]) {
			case XHPROF_STRING_PARAM:
				$p = xhprofGetStringParam($k, $v[1]);
				break;
			case XHPROF_UINT_PARAM:
				$p = xhprofGetUintParam($k, $v[1]);
				break;
			case XHPROF_FLOAT_PARAM:
				$p = xhprofGetFloatParam($k, $v[1]);
				break;
			case XHPROF_BOOL_PARAM:
				$p = xhprofGetBoolParam($k, $v[1]);
				break;
			default:
				xhprofError("Invalid param type passed to xhprof_param_init: "
				             . $v[0]);
				exit();
		}
 ...
 * @author Kannan
 */
function xhprofGetMatchingFunctions($q, $xhprof_data) {
